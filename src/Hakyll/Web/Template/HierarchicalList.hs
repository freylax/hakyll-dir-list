...
module Hakyll.Web.Template.DirectoryList
       (
         dirListField         
       ) where
...
getItemTreeAList ( ItemTree i [] pid _ ) level btags etags =
...
dirListField key c xs = listField key ( c' `mappend` c) pages' 
  where
    ...
    aList' = map (\(item,c)->(itemIdentifier item,c)) <$> aList
